...
def main(args):
    ...
    logger.info('train transforms: {}'.format(video_augmentation.transforms))
    logger.info('val transforms: {}'.format(video_augmentation.transforms))
    ...
    classes = torch.cat([clip.tokenize(c) for i, c in enumerate(train_data.classes)])
...
def train(model, train_loader, optimizer, criterion, scaler,
    ...
    for i, (video, class_id) in enumerate(train_loader):
        ...
        with autocast():
            logits = model(video, text_embedding)  # B 400
...
def validate(epoch, val_loader, device, model, config, text_embedding, logger):
    ...
    with torch.no_grad():
        for i, (video, class_id) in enumerate(val_loader):
            ...
            video_embedding = model.module.encode_image(video)
